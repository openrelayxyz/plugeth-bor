...
func gethToUtilsHeader(header *types.Header) *ptypes.Header {
	...
	return &ptypes.Header{
		...
		// WithdrawalsHash: (*core.Hash)(header.WithdrawalsHash), --this is commented out in bor-plugeth only
	}
}
func utilsToGethHeader(header *ptypes.Header) *types.Header {
	...
	return &types.Header{
		...
		BaseFee: header.BaseFee,
		// WithdrawalsHash: (*common.Hash)(header.WithdrawalsHash), -- this is commented in bor-plugeth only
	}
}
...
func gethToUtilsWithdrawals() []*ptypes.Withdrawal { // this is a bor-plugeth specific dummy function
	return nil
}
...
func gethToUtilsBlock(block *types.Block) *ptypes.Block {
	if block == nil { return nil }
	return ptypes.NewBlockWithHeader(gethToUtilsHeader(block.Header())).WithBody(gethToUtilsTransactions(block.Transactions()), gethToUtilsHeaders(block.Uncles())) // dropping withdrawals is bor-plugeth specific implementation
}
...
func utilsToGethBlock(block *ptypes.Block) *types.Block {
	if block == nil { return nil }
	return types.NewBlockWithHeader(utilsToGethHeader(block.Header())).WithBody(utilsToGethTransactions(block.Transactions()), utilsToGethHeaders(block.Uncles())) // dropping withdrawals is bor-plugeth specific implementation
}
...
// 	}
// 	return pwithdrawals
// } -- commenting this function is bor-plugeth specific implenentation
...
func (ew *engineWrapper) Seal(ctx context.Context, chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
