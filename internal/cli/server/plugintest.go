...
func (p *plugintest) Blockchain() {
	log.Error("inside of blockchain function")
	client := stack.Attach()

	for {
		var chainCall bool
		if err := client.Call(&chainCall, "admin_importChain", "./test/midBorChain.gz"); err != nil {
			log.Error("Error calling importChain from client, plugintest", "err", err)
			time.Sleep(2 * time.Second)
			continue
		}
		if chainCall {
			log.Info("Chain successfully imported")
			break
		}
		log.Warn("Failed to import chain, retrying...")
		time.Sleep(2 * time.Second)
	}
...
func (p *plugintest) RunTest(ctx context.Context) {
	log.Error("running setTrieFlush")
	err := p.callRPC("xplugeth_setTrieFlushInterval", []interface{}{"1h30m"})
	if err != nil {
	...
	if modifiedInterval <= nodeInterval {
		...
		log.Info("setTrieFlushTest passed")
	}
}
...
func stateDataDecompress() (map[uint64]map[string]interface{}, error) {
	file, err := os.ReadFile("./test/state-control.json.gz")
	if err != nil {
		log.Error("cannot read file control.json.gz")
		return nil, err
	}
